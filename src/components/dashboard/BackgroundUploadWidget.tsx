    ...
    <div className="fixed bottom-5 right-5 z-40 animate-in slide-in-from-bottom-5 fade-in duration-300">
      {isMinimized ? (
        ...
        <button
          ...
          className="flex items-center gap-2.5 px-4 py-3 rounded-full bg-slate-900/95 border border-indigo-500/40 text-white shadow-2xl backdrop-blur-md hover:bg-slate-800 transition cursor-pointer active:scale-95 group"
        ...
        <div className="w-80 sm:w-96 rounded-2xl glass-card border border-indigo-500/30 p-4 space-y-3.5 shadow-2xl backdrop-blur-xl relative bg-slate-950/90">
          {/* Header */}
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h4 className="text-xs font-extrabold text-white">Proses Unggah Background</h4>
                <p className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5">
                  <Folder className="w-3 h-3 text-indigo-400 inline flex-shrink-0" />
                  <span className="truncate max-w-[140px]">Target: {targetFolderName || 'Root'}</span>
                </p>
              </div>
            </div>
            ...
            <p className="text-[10px] text-slate-400 leading-tight truncate" title={currentFileName || statusMessage}>
              {statusMessage ? (
                ...
                <span className="text-purple-300/90 font-medium flex items-center gap-1">
                  ...
                  Mengunggah berkas ke Google Drive ({activePercent}%)...
                </span>
              ) : (
          ...
          <div className="flex items-center justify-between pt-1 border-t border-slate-800/60 text-[10px] text-slate-400">
            <span>Anda bebas menelusuri folder lain</span>
            <button
              onClick={onCancelUpload}
