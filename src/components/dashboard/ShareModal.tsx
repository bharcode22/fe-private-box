      ...
      <div className="relative w-full max-w-lg max-h-[90vh] bg-slate-900 border border-slate-800/90 rounded-3xl shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 sm:px-6 py-3.5 border-b border-slate-800/80 bg-slate-900/90 sticky top-0 z-10">
          <div className="flex items-center space-x-3 overflow-hidden min-w-0 flex-1 pr-2">
            <div className="p-2 sm:p-2.5 rounded-2xl bg-gradient-to-tr from-indigo-500/20 to-purple-500/20 border border-indigo-500/30 text-indigo-400 flex-shrink-0 shadow-md shadow-indigo-500/10">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[11px] sm:text-xs text-slate-400 truncate flex items-center gap-1 mt-0.5" title={modalData.name}>
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4 sm:space-y-5">
          ...
          <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80 space-y-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <button
                ...
                className="text-[11px] text-indigo-400 hover:text-indigo-300 font-bold flex items-center gap-1 cursor-pointer transition-colors whitespace-nowrap flex-shrink-0 bg-indigo-500/10 px-2 py-1 rounded-lg border border-indigo-500/20 active:scale-95 disabled:opacity-50"
              >
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 px-3.5 py-2 rounded-xl bg-slate-950 border border-slate-800 focus:border-indigo-500 font-mono text-sm font-bold text-indigo-300 uppercase tracking-wider focus:outline-none min-w-0"
              />
              <button
                type="button"
                onClick={() => {
          ...
          <div className="p-4 border-t border-slate-800/80 sticky bottom-0 bg-slate-900 z-10 flex items-center justify-end gap-2">
            <button
              type="button"
              onClick={onClose}
              disabled={saving}
              className="flex-1 sm:flex-none px-4 py-2.5 rounded-xl bg-slate-800/90 hover:bg-slate-700/80 text-slate-200 hover:text-white font-bold text-xs border border-slate-700/80 transition-all shadow-sm cursor-pointer active:scale-95 flex items-center justify-center gap-1.5 disabled:opacity-50"
            >
